...
class AdminItemController extends Controller
{
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        $items = Item::where('no', 'like', "%{$q}%")
            ...
            ->orderBy('name')
            ->limit(30)
            ->get(['id', 'no', 'name', 'images', 'video_url']);

        return response()->json($items);
    }

    public function updateVideo(Request $request, Item $item): RedirectResponse
    {
        $request->validate([
        ...
        $item->update(['video_url' => $request->input('video_url') ?: null]);

        return redirect()->back()->with('message', 'Video link updated.');
    }
}
