...
<template>
    ...
    <template v-if="$page.props?.user">
        ...
        <div v-else class="rounded-2xl bg-brand-50 border border-brand-100 px-4 py-3">
            <div class="flex items-center justify-between gap-3">
                <div class="flex items-center gap-2 text-brand-600 text-sm font-medium">
                    <i class="pi pi-bell text-brand-400"></i>
                    შეტყობინება ჩართულია
                </div>
                <button
                    @click="router.delete(route('stock-notifications.unsubscribe', item.slug))"
                    class="text-xs text-gray-400 hover:text-red-500 transition-colors cursor-pointer shrink-0"
                >
                    გაუქმება
                </button>
            </div>
            <p class="text-xs text-brand-400 mt-1.5">
