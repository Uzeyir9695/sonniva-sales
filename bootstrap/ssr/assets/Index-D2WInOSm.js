...
import { _ as _sfc_main$3 } from "./SwiperCarousel-CbaFFUjQ.js";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(categories), (category) => {
        _push(ssrRenderComponent(unref(Link), mergeProps({
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<p class="flex items-center gap-3"${_scopeId}><img${ssrRenderAttr("src", `categories-icons/${category.code}.png?v=${unref(CATEGORY_ICON_VERSION)}`)}${ssrRenderAttr("alt", category.name)} class="w-12 h-12 object-cover rounded-lg"${_scopeId}><span class="${ssrRenderClass([[category.name === "Sale" ? "text-brand-500" : activeCategory.value?.name === category.name ? "text-brand-400" : "text-gray-700"], "text-sm font-medium"])}"${_scopeId}>${ssrInterpolate(category.name)}</span></p><i class="${ssrRenderClass([activeCategory.value?.name === category.name ? "text-brand-400" : "text-gray-400", "pi pi-chevron-right text-xs"])}"${_scopeId}></i>`);
            } else {
              return [
                createVNode("p", { class: "flex items-center gap-3" }, [
                  createVNode("img", {
                    src: `categories-icons/${category.code}.png?v=${unref(CATEGORY_ICON_VERSION)}`,
                    alt: category.name,
...
//# sourceMappingURL=Index-D2WInOSm.js.map
