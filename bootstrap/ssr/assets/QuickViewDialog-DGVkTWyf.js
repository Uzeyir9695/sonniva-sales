...
import { ref, computed, mergeProps, useSSRContext, reactive, watch, unref, toValue, withCtx, createTextVNode, createVNode, openBlock, createBlock, toDisplayString, Fragment, renderList, createCommentVNode } from "vue";
...
function getDisplayPrice(item) {
  return item?.unit_price == 0 && item?.prices?.length ? item.prices[0]?.price ?? null : item?.unit_price;
}
function getDisplayUOM(item) {
  return item?.unit_price == 0 && item?.prices?.length ? item.prices[0]?.UOM ?? null : null;
}
...
//# sourceMappingURL=QuickViewDialog-DGVkTWyf.js.map
