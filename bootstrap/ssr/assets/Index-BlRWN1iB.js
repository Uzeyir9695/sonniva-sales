...
import { u as useWishlist } from "./QuickViewDialog-DXL2MTuj.js";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "px-4 sm:px-6 lg:px-8 py-10" }, _attrs))}>`);
      if (filteredItems.value?.data.length) {
        _push(`<div class="flex items-center justify-between mb-8"><div><h1 class="text-lg sm:text-2xl font-bold text-gray-900">${ssrInterpolate(_ctx.$t("wishlist.title"))}</h1><p class="text-gray-500 text-sm mt-1">${ssrInterpolate(filteredItems.value.data.length)} ${ssrInterpolate(filteredItems.value.data.length === 1 ? _ctx.$t("wishlist.savedOne") : _ctx.$t("wishlist.savedMany"))}</p></div></div>`);
      } else {
        _push(`<!---->`);
      }
      if (!filteredItems.value?.data.length) {
        _push(`<div class="flex flex-col items-center justify-center py-24 text-center"><i class="pi pi-heart text-gray-300 text-6xl mb-6"></i><h2 class="text-lg font-semibold text-gray-700 mb-2">${ssrInterpolate(_ctx.$t("wishlist.emptyHeading"))}</h2><p class="text-gray-500 text-sm mb-6 max-w-xs">${ssrInterpolate(_ctx.$t("wishlist.emptyText"))}</p>`);
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`${ssrInterpolate(_ctx.$t("common.startShopping"))}`);
            } else {
              return [
                createTextVNode(toDisplayString(_ctx.$t("common.startShopping")), 1)
              ];
            }
          }),
          _: 1
...
//# sourceMappingURL=Index-BlRWN1iB.js.map
