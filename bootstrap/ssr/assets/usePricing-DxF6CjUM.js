...
function allowedTiers(item, isVip, canViewWholesales) {
  return (item?.prices ?? []).filter((p) => isVip || p.priceGroup !== "VIP").filter((p) => !wholesaleTierGated(item, canViewWholesales) || p.priceGroup !== "Wholesales");
}
...
function calculateTierPrice(item, qty, selectedUOM = null, isVip = false, canViewWholesales = true) {
  if (!item) return 0;
...
function activeDiscountType(item, qty, selectedUOM = null, isVip = false, canViewWholesales = true) {
  if (!item) return null;
...
//# sourceMappingURL=usePricing-DxF6CjUM.js.map
