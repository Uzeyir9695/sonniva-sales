    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (imagesFor("main").length) {
        ...
        ssrRenderList(imagesFor("main"), (img) => {
          _push(`<div class="rounded-xl overflow-hidden bg-gray-100 shadow-md flex flex-col"><div class="relative group aspect-video"><img${ssrRenderAttr("src", img.image_url)} alt="banner" class="w-full h-full object-cover"><span class="absolute bottom-0 left-0 right-0 px-2 py-1 text-[11px] font-medium text-white bg-black/60 truncate">${ssrInterpolate(img.item?.name ?? "No item linked")}</span><button class="absolute top-1.5 right-1.5 flex items-center justify-center w-7 h-7 rounded-lg bg-black/60 text-white opacity-0 group-hover:opacity-100 transition-all hover:bg-red-600"><i class="pi pi-trash text-xs"></i></button></div><div class="flex items-center gap-2 px-2 py-1.5 bg-white border-t border-gray-100">`);
          if (img.mobile_image_url) {
            _push(`<!--[--><img${ssrRenderAttr("src", img.mobile_image_url)} alt="mobile" class="w-6 h-8 object-cover rounded shrink-0"><span class="text-[11px] font-medium text-gray-600 flex items-center gap-1"><i class="pi pi-mobile text-xs"></i> Mobile set </span><button class="ml-auto text-[11px] font-medium text-red-500 hover:text-red-600"> Remove </button><!--]-->`);
          } else {
            _push(`<label class="${ssrRenderClass([uploading.value[`mobile-${img.id}`] ? "opacity-60 pointer-events-none" : "", "flex items-center gap-1.5 text-[11px] font-medium text-brand-600 hover:text-brand-700 cursor-pointer"])}"><i class="pi pi-upload text-xs"></i> ${ssrInterpolate(uploading.value[`mobile-${img.id}`] ? "Uploading..." : "Add mobile version")} <input type="file" accept="image/*" class="hidden"></label>`);
          }
          _push(`</div></div>`);
        });
        _push(`<!--]--></div>`);
...
//# sourceMappingURL=HeroBannersTab-UCD4IPT5.js.map
