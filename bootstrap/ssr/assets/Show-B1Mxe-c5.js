...
import { s as script$2, a as script$3, b as script$4 } from "./index-CwYp4h2j.js";
import { _ as _sfc_main$1, s as script$1 } from "./Breadcrumbs-r-JAysNq.js";
import { ref, onMounted, computed, watch, unref, withCtx, createVNode, openBlock, createBlock, createCommentVNode, mergeProps, toDisplayString, createTextVNode, Fragment, renderList, useSSRContext } from "vue";
import { ssrRenderComponent, ssrRenderAttr, ssrRenderClass, ssrInterpolate, ssrRenderAttrs, ssrGetDirectiveProps, ssrRenderList, ssrIncludeBooleanAttr } from "vue/server-renderer";
import { usePage, Head, Deferred } from "@inertiajs/vue3";
import { useClipboard } from "@vueuse/core";
import _sfc_main$6 from "./SimilarItems-QfUNfbfr.js";
import _sfc_main$2 from "./ItemGallery-Dn36N-Eg.js";
import { u as usePricing, b as _sfc_main$3, _ as _sfc_main$4 } from "./QuickViewDialog-BwjyUAul.js";
import { _ as _sfc_main$5 } from "./StockNotifyButton-BihFmX1g.js";
import { _ as _sfc_main$7 } from "./ItemCard-YMo0znvQ.js";
...
import "./SwiperCarousel-DXDT2oI3.js";
import "swiper/vue";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<span>${ssrInterpolate(inStock.value ? "მარაგშია" : "მარაგში არაა")}</span></div><div class="flex items-center gap-3">`);
      ...
      if (unref(copied)) {
        _push(`<span class="text-emerald-500 text-xs">დაკოპირებულია</span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "flex items-center justify-center w-7 h-7 border border-gray-200 rounded-lg shadow-md hover:bg-gray-50 hover:border-gray-300 transition-all duration-150 cursor-pointer" }, ssrGetDirectiveProps(_ctx, _directive_tooltip, "დააკოპირე პროდუქტის კოდი", void 0, { bottom: true })))}>`);
      if (unref(copied)) {
        _push(`<i class="pi pi-check text-emerald-500 text-xs"></i>`);
      } else {
        _push(`<i class="pi pi-copy text-gray-400 text-sm"></i>`);
      }
      _push(`</div></div></div><h1 class="sm:text-xl font-semibold text-gray-900 leading-snug mb-2">${ssrInterpolate(__props.item.name)}</h1><div class="flex items-center gap-3 mb-8"><span class="text-lg sm:text-2xl font-bold text-brand-500 tracking-tight">${ssrInterpolate(unref(isPackageItem) ? selectedEntry.value?.price : __props.item.unit_price)} ₾ </span>`);
      ...
      _push(`</div>`);
      if (_ctx.$page.props.user?.can_view_inventory) {
        _push(ssrRenderComponent(unref(Deferred), { data: "inventory" }, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="flex items-center gap-3 mt-4 mb-6"${_scopeId}><div class="flex-1 flex items-center justify-between bg-gray-50 border border-gray-100 rounded-2xl px-4 py-3"${_scopeId}><div class="flex items-center gap-2"${_scopeId}><i class="pi pi-warehouse text-brand-500 text-sm"${_scopeId}></i><span class="text-xs text-gray-500"${_scopeId}>დიდუბის ფილიალი</span></div><span class="text-sm font-semibold text-gray-800"${_scopeId}>${ssrInterpolate(__props.inventory.shop1Total)}</span></div><div class="flex-1 flex items-center justify-between bg-gray-50 border border-gray-100 rounded-2xl px-4 py-3"${_scopeId}><div class="flex items-center gap-2"${_scopeId}><i class="pi pi-warehouse text-brand-500 text-sm"${_scopeId}></i><span class="text-xs text-gray-500"${_scopeId}>ავჭალლის ფილიალი</span></div><span class="text-sm font-semibold text-gray-800"${_scopeId}>${ssrInterpolate(__props.inventory.shop2Total)}</span></div></div>`);
            } else {
              return [
                createVNode("div", { class: "flex items-center gap-3 mt-4 mb-6" }, [
                  createVNode("div", { class: "flex-1 flex items-center justify-between bg-gray-50 border border-gray-100 rounded-2xl px-4 py-3" }, [
                    createVNode("div", { class: "flex items-center gap-2" }, [
                      createVNode("i", { class: "pi pi-warehouse text-brand-500 text-sm" }),
                      createVNode("span", { class: "text-xs text-gray-500" }, "დიდუბის ფილიალი")
                    ]),
                  ...
                  createVNode("div", { class: "flex-1 flex items-center justify-between bg-gray-50 border border-gray-100 rounded-2xl px-4 py-3" }, [
                    createVNode("div", { class: "flex items-center gap-2" }, [
                      createVNode("i", { class: "pi pi-warehouse text-brand-500 text-sm" }),
                      createVNode("span", { class: "text-xs text-gray-500" }, "ავჭალლის ფილიალი")
                    ]),
      ...
      ssrRenderList(__props.item.prices, (price) => {
        _push(`<!--[-->`);
        if (_ctx.$page.props.user?.can_view_wholesales || price.priceGroup === "VIP" && _ctx.$page.props.user?.can_view_vip) {
          _push(`<div class="flex items-center justify-between bg-gray-50 border border-gray-100 rounded-2xl px-4 py-3 mb-2 hover:border-brand-200 hover:bg-brand-50/30 transition-all duration-150"><div class="flex-1"><p class="text-sm font-semibold text-gray-800 mb-1">${ssrInterpolate(price.priceGroup === "VIP" ? "VIP" : price.priceGroup === "Wholesales" ? "საბითუმო" : "საცალო")}</p>`);
          if (price.custMinQuantity > 0) {
            _push(`<div class="flex items-center gap-1.5"><i class="pi pi-box text-gray-400 text-xs"></i><p class="text-xs text-gray-400"> მინ. რაოდენობა: <span class="text-gray-600 font-medium">${ssrInterpolate(price.custMinQuantity)}</span></p></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div><div class="text-right ml-4"><p class="text-xs text-gray-400 mb-0.5">${ssrInterpolate(__props.item.unit_price === "0.00" ? "შეკვრის ფასი" : "ცალის ფასი")}</p><p class="text-brand-500 font-bold text-xl">${ssrInterpolate(price.price)}<span class="text-sm ml-0.5">₾</span></p></div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<!--]-->`);
      ...
      if (!inStock.value) {
        _push(`<button class="w-full py-2.5 rounded-2xl max-sm:text-sm flex items-center justify-center gap-2 bg-slate-700 cursor-pointer border border-slate-800 text-white font-bold hover:bg-slate-800 active:scale-[0.98] transition-all"><i class="pi pi-file-edit"></i> მოითხოვე შეკვეთა </button>`);
      } else {
        _push(`<!---->`);
      }
      ...
      _push(ssrRenderComponent(_component_Panel, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<ul class="space-y-2 text-xs text-gray-600"${_scopeId}><li class="flex gap-2"${_scopeId}><span class="text-brand-500"${_scopeId}>→</span><span${_scopeId}><strong${_scopeId}>თვითგატანა ოფისიდან:</strong> უფასო. გატანის წერტილი <a href="https://maps.app.goo.gl/3YwH55CnhUUfJoYQ9" target="_blank" class="text-blue-500 hover:underline"${_scopeId}> რუკაზე </a></span></li><li class="flex gap-2"${_scopeId}><span class="text-brand-500"${_scopeId}>→</span> <span${_scopeId}>მიწოდება თბილისში: 100+ ₾ უფასო </span></li><li class="flex gap-2"${_scopeId}><span class="text-brand-500"${_scopeId}>→</span><span${_scopeId}> მიწოდება რეგიონებში: <a href="#" class="text-blue-500 hover:underline"${_scopeId}>იხილეთ მიწოდების ტარიფები</a></span></li><li class="flex gap-2"${_scopeId}><span class="text-brand-500"${_scopeId}>→</span> <span${_scopeId}>13:00-მდე გაფორმებულ შეკვეთებს თბილისში გაწვდით იმავე დღეს!</span></li><li class="flex gap-2"${_scopeId}><span class="text-brand-500"${_scopeId}>→</span> <span${_scopeId}>მიტანის სერვისის ფარგლებში პროდუქციის მანქანიდან ჩამოტვირთვა და სართულზე ატანა არ შედის მომსახურებაში.</span></li></ul>`);
          } else {
            return [
              createVNode("ul", { class: "space-y-2 text-xs text-gray-600" }, [
                ...
                createVNode("li", { class: "flex gap-2" }, [
                  createVNode("span", { class: "text-brand-500" }, "→"),
                  createTextVNode(),
                  createVNode("span", null, "მიწოდება თბილისში: 100+ ₾ უფასო ")
                ]),
                createVNode("li", { class: "flex gap-2" }, [
                  createVNode("span", { class: "text-brand-500" }, "→"),
                  createVNode("span", null, [
...
//# sourceMappingURL=Show-B1Mxe-c5.js.map
