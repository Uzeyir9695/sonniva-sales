...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-[calc(100vh-96px)] bg-gray-50 flex items-center justify-center p-6" }, _attrs))}><div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 max-w-md w-full text-center"><div class="w-16 h-16 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-6"><i class="pi pi-check text-white text-2xl"></i></div><h1 class="text-xl font-bold text-gray-900 mb-2">${ssrInterpolate(_ctx.$t("payment.successTitle"))}</h1>`);
      if (__props.invoiceNumber) {
        _push(`<p class="text-sm text-gray-500 mb-1">${ssrInterpolate(_ctx.$t("common.orderNumber"))}</p>`);
      } else {
        _push(`<!---->`);
      }
      if (__props.invoiceNumber) {
      ...
      _push(`<p class="text-sm text-gray-400 mb-8">${ssrInterpolate(_ctx.$t("payment.confirmationEmailSent"))}</p><div class="space-y-3">`);
...
//# sourceMappingURL=Success-B3oswBWU.js.map
