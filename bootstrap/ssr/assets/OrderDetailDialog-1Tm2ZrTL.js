...
import { ref, mergeProps, withCtx, unref, openBlock, createBlock, createVNode, toDisplayString, createTextVNode, createCommentVNode, Fragment, renderList, useSSRContext } from "vue";
          ...
          if (_push2) {
            if (loading.value) {
              ...
              ssrRenderList(order.value.items, (data) => {
                _push2(`<div class="border border-gray-200 rounded-xl p-3 flex flex-col gap-2"${_scopeId}><div class="flex items-start justify-between gap-2"${_scopeId}><div class="flex flex-col gap-0.5 min-w-0"${_scopeId}><span class="font-semibold text-gray-800"${_scopeId}>${ssrInterpolate(data.item_name)}</span><span class="text-xs text-gray-400 font-mono"${_scopeId}>${ssrInterpolate(data.item_no)}</span></div><span class="text-xs font-semibold text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full shrink-0"${_scopeId}>x${ssrInterpolate(data.quantity)}</span></div>`);
...
//# sourceMappingURL=OrderDetailDialog-1Tm2ZrTL.js.map
