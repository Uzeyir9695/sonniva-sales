...
import { resolveDirective, openBlock, createElementBlock, mergeProps, createBlock, resolveDynamicComponent, Fragment, createElementVNode, toDisplayString, createCommentVNode, normalizeProps, withDirectives, resolveComponent, withCtx, createVNode, TransitionGroup, renderList, computed, unref, useSSRContext, ref, createTextVNode, watch, onMounted, onUnmounted, Transition } from "vue";
import { ssrRenderComponent, ssrInterpolate, ssrRenderAttrs, ssrRenderAttr, ssrRenderList, ssrRenderClass, ssrRenderTeleport, ssrGetDirectiveProps } from "vue/server-renderer";
...
import { _ as _sfc_main$9, a as _sfc_main$a, u as useWishlist } from "./QuickViewDialog-N1KpTX5R.js";
import { u as useCart } from "./useCart-CLT7fb2x.js";
import { b as getDisplayUOM, g as getDisplayPrice, e as getOriginalPrice } from "./usePricing-BqOIsB71.js";
      ...
      if (isAuthenticated.value) {
        _push(ssrRenderComponent(unref(Link), mergeProps({
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<i class="pi pi-sign-out text-sm ml-1 text-gray-400"${_scopeId}></i><span class="max-sm:hid"${_scopeId}>${ssrInterpolate(_ctx.$t("nav.signOut"))}</span>`);
            } else {
              return [
                createVNode("i", { class: "pi pi-sign-out text-sm ml-1 text-gray-400" }),
                createVNode("span", { class: "max-sm:hid" }, toDisplayString(_ctx.$t("nav.signOut")), 1)
              ];
            }
          }),
          _: 1
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({
        ...
        ref: rootRef,
        class: "relative"
      }, _attrs))}><button type="button" class="flex items-center justify-center md:w-8 md:h-8 lg:w-10 lg:h-10 gap-1.5 rounded-full text-gray-600 hover:bg-gray-100 transition-all cursor-pointer"><img${ssrRenderAttr("src", flagMap[current.value])} class="w-6 h-4 object-cover rounded-xs shrink-0" alt=""><span class="sm:hidden text-sm font-medium text-gray-600">${ssrInterpolate(fullNameMap[current.value])}</span></button>`);
      if (open.value) {
        ...
        ssrRenderList(LOCALES, (locale) => {
          _push(`<button class="${ssrRenderClass([current.value === locale ? "text-brand-500 font-semibold bg-brand-50/50" : "text-gray-700", "w-full flex items-center gap-2.5 px-4 py-2.5 text-sm cursor-pointer hover:bg-gray-50 transition-colors"])}"><img${ssrRenderAttr("src", flagMap[locale])} class="w-5 h-3.5 object-cover rounded-xs shrink-0" alt=""><span class="hidden sm:inline">${ssrInterpolate(nameMap[locale])}</span><span class="sm:hidden">${ssrInterpolate(fullNameMap[locale])}</span>`);
...
_sfc_main$7.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Shared/components/LocaleSwitcher.vue");
...
const _sfc_main$6 = {
  ...
  setup(__props, { expose: __expose }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--[--><button class="flex items-center cursor-pointer bg-brand-400 hover:bg-brand-500 text-gray-900 font-semibold p-2.5 h-fit rounded-full transition-colors" data-v-da8ceabb><i class="pi pi-th-large text-sm text-white" data-v-da8ceabb></i></button>`);
      ssrRenderTeleport(_push, (_push2) => {
        if (drawerOpen.value) {
          _push2(`<div class="fixed inset-0 z-40 bg-black/30 backdrop-blur-sm" data-v-da8ceabb></div>`);
        } else {
          _push2(`<!---->`);
        }
        if (drawerOpen.value) {
          _push2(`<div class="fixed top-16 sm:top-20 left-0 z-50 h-[calc(100dvh-60px)] w-full sm:w-80 bg-white shadow-2xl flex flex-col" data-v-da8ceabb><div class="bg-gray-50 shado w-full sm:w-xs flex items-center justify-between px-4 py-4 border-y border-gray-100 sm:hidden" data-v-da8ceabb>`);
          _push2(ssrRenderComponent(_sfc_main$7, null, null, _parent));
          _push2(ssrRenderComponent(_sfc_main$8, null, null, _parent));
          _push2(`</div><div class="flex items-center gap-3 px-4 py-4 border-b border-gray-100" data-v-da8ceabb><div class="w-8 h-8 flex items-center justify-center rounded-full cursor-pointer hover:bg-gray-100 transition-colors shrink-0" data-v-da8ceabb><i class="pi pi-arrow-left text-gray-600 text-sm" data-v-da8ceabb></i></div><span class="font-semibold text-gray-900 text-sm" data-v-da8ceabb>${ssrInterpolate(currentTitle.value)}</span><button class="ml-auto w-8 h-8 flex items-center justify-center cursor-pointer rounded-full hover:bg-gray-100 shrink-0" data-v-da8ceabb><i class="pi pi-times text-gray-400 text-sm" data-v-da8ceabb></i></button></div><div class="small-device-menu flex-1 overflow-y-auto" data-v-da8ceabb>`);
          if (fadeVisible.value) {
            _push2(`<div class="p-2" data-v-da8ceabb>`);
            if (stack.value.length > 0) {
              _push2(ssrRenderComponent(unref(Link), {
                ...
                default: withCtx((_, _push3, _parent2, _scopeId) => {
                  if (_push3) {
                    _push3(`<i class="pi pi-th-large text-xs" data-v-da8ceabb${_scopeId}></i> ${ssrInterpolate(_ctx.$t("common.viewAll"))}`);
                  } else {
                    return [
            ...
            if (stack.value.length === 0) {
              ...
              ssrRenderList(currentItems.value, (item) => {
                _push2(`<div class="w-full px-5 py-2 cursor-pointer rounded-xl text-gray-800 hover:bg-gray-50 hover:text-brand-400 transition-colors" data-v-da8ceabb><button${ssrRenderAttrs(mergeProps({
                  style: { "--p-ripple-background": "rgba(251, 191, 36, 0.3)" },
                  class: "w-full flex items-center justify-between cursor-pointer text-left"
                }, ssrGetDirectiveProps(_ctx, _directive_ripple)))} data-v-da8ceabb><div class="flex items-center gap-x-3" data-v-da8ceabb><img${ssrRenderAttr("src", `/categories-icons/${item.code}-${unref(CATEGORY_ICON_VERSION)}.png`)}${ssrRenderAttr("alt", item.name)} class="w-10 h-10 object-cover rounded-lg" data-v-da8ceabb><span class="text-sm font-medium text-inherit" data-v-da8ceabb>${ssrInterpolate(item.name)}</span></div><i class="pi pi-chevron-right text-xs text-inherit" data-v-da8ceabb></i></button></div>`);
              });
              _push2(`<!--]-->`);
            } else if (stack.value.length === 1) {
              _push2(`<div class="grid grid-cols-2 gap-3" data-v-da8ceabb><!--[-->`);
              ssrRenderList(currentItems.value, (item) => {
                _push2(`<!--[-->`);
                if (hasChildren(item)) {
                  _push2(`<div class="flex flex-col items-center text-center w-full px-2.5 py-2.5 space-y-1 cursor-pointer rounded-xl text-gray-800 shadow-sm hover:bg-gray-50 hover:text-brand-400 transition-colors" data-v-da8ceabb>`);
                  if (item.image) {
                    _push2(`<img${ssrRenderAttr("src", `${item.storage_path}/${item.image}`)}${ssrRenderAttr("alt", item.name)} class="w-32 h-20 object-cover rounded-lg mx-auto mb-2" data-v-da8ceabb>`);
                  } else {
                    _push2(`<!---->`);
                  }
                  ...
                  }, ssrGetDirectiveProps(_ctx, _directive_ripple)))} data-v-da8ceabb><span class="text-sm font-medium text-inherit" data-v-da8ceabb>${ssrInterpolate(item.name)} ${ssrInterpolate(item.items_count ? `(${item.items_count})` : item.items?.length < 1 ? "(0)" : "")}</span><i class="pi pi-chevron-right text-xs text-amber-400" data-v-da8ceabb></i></button></div>`);
                } else {
                  _push2(ssrRenderComponent(unref(Link), mergeProps({
                    ...
                    default: withCtx((_, _push3, _parent2, _scopeId) => {
                      if (_push3) {
                        if (item.image) {
                          _push3(`<img${ssrRenderAttr("src", `${item.storage_path}/${item.image}`)}${ssrRenderAttr("alt", item.name)} class="w-32 h-20 object-cover rounded-lg mx-auto mb-2" data-v-da8ceabb${_scopeId}>`);
                        } else {
                          _push3(`<!---->`);
                        }
                        _push3(`<span class="text-sm font-medium" data-v-da8ceabb${_scopeId}>${ssrInterpolate(item.name)} ${ssrInterpolate(item.items_count ? `(${item.items_count})` : "(0)")}</span>`);
                      } else {
                        return [
              ...
              ssrRenderList(currentItems.value, (item) => {
                _push2(ssrRenderComponent(unref(Link), mergeProps({
                  ...
                  default: withCtx((_, _push3, _parent2, _scopeId) => {
                    if (_push3) {
                      _push3(`<span class="text-sm font-medium text-inherit" data-v-da8ceabb${_scopeId}>${ssrInterpolate(item.name)} ${ssrInterpolate(item.items_count ? `(${item.items_count})` : "(0)")}</span>`);
                    } else {
                      return [
...
const SmallDeviceMegaMenu = /* @__PURE__ */ _export_sfc(_sfc_main$6, [["__scopeId", "data-v-da8ceabb"]]);
const _sfc_main$5 = {
  __name: "SearchBar",
  __ssrInlineRender: true,
  props: {
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="relative w-full"><form action="/search" method="get" class="${ssrRenderClass([showDropdown.value ? "rounded-b-none border-2 border-brand-400" : "", "flex items-center rounded-xl px-4 h-11 gap-3 transition-all focus-within:border-2 border border-brand-400"])}"><button type="submit"${ssrRenderAttr("aria-label", _ctx.$t("common.search"))} class="shrink-0 flex items-center justify-center cursor-pointer">`);
      if (!loading.value) {
        _push(`<i class="pi pi-search text-gray-400 text-sm"></i>`);
      } else {
        _push(`<i class="pi pi-spinner pi-spin text-brand-400 text-sm"></i>`);
      }
      _push(`</button><input${ssrRenderAttr("value", query.value)} type="search" name="q"${ssrRenderAttr("placeholder", placeholderText.value)} class="w-full bg-transparent outline-none text-sm text-gray-700 placeholder-gray-400">`);
      if (query.value) {
        _push(`<button type="button" class="text-gray-400 hover:text-gray-600 shrink-0 cursor-pointer"><i class="pi pi-times text-xs"></i></button>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</form>`);
      if (showDropdown.value) {
        ...
        if (results.value.length > 0) {
          _push(`<div class="flex flex-col sm:flex-row items-center justify-between gap-2 px-4 py-2.5 bg-gray-50 border-b border-gray-100"><div class="flex items-center justify-center gap-2 sticky top-0 text-xs font-medium text-gray-500"><i class="pi pi-search text-xs"></i> ${ssrInterpolate(_ctx.$t("search.foundCount", { count: results.value.length }))}</div><button class="flex items-center justify-center rounded-lg gap-x-2 sticky top-0 px-4 py-1.5 sm:py-2.5 sm:bg-gray-50 bg-slate-100 hover:bg-gray-100 cursor-pointer transition-colors text-xs font-medium text-gray-500"><i class="pi pi-eye text-xs"></i><span>${ssrInterpolate(_ctx.$t("common.viewAll"))}</span></button></div>`);
        } else {
          _push(`<!---->`);
        }
        if (results.value.length === 0) {
          _push(`<div class="flex flex-col items-center justify-center gap-2 py-10 text-gray-400"><i class="pi pi-search text-2xl"></i><p class="text-sm font-medium">${ssrInterpolate(_ctx.$t("search.noResults", { query: query.value }))}</p><p class="text-xs">${ssrInterpolate(_ctx.$t("search.tryAnother"))}</p></div>`);
        } else {
          _push(`<ul><!--[-->`);
          ssrRenderList(results.value.slice(0, visibleCount.value), (item) => {
            ...
            if (unref(getDisplayUOM)(item)) {
              _push(`<span class="text-xs text-blue-400">${ssrInterpolate(_ctx.$t("search.pack"))}</span>`);
            } else {
              _push(`<!---->`);
            }
            ...
            if (unref(getOriginalPrice)(item)) {
              _push(`<span class="text-xs text-red-500 line-through">${ssrInterpolate(unref(getOriginalPrice)(item))} ₾</span>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div><span class="${ssrRenderClass([item.inventory > 0 ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-500", "text-xs px-2 py-0.5 rounded-full font-medium shrink-0"])}">${ssrInterpolate(item.inventory > 0 ? _ctx.$t("common.inStock") : _ctx.$t("common.outOfStock"))}</span></div><div class="flex items-center gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity shrink-0"><button${ssrRenderAttrs(mergeProps({ class: "w-5 sm:w-7 h-5 sm:h-7 rounded-lg cursor-pointer flex items-center justify-center text-gray-400 hover:text-rose-500 hover:bg-rose-50 transition-colors" }, ssrGetDirectiveProps(_ctx, _directive_tooltip, _ctx.$t("search.wishlist"), void 0, { top: true })))}><i class="pi pi-heart"></i></button><button${ssrRenderAttrs(mergeProps({
            ...
            }, ssrGetDirectiveProps(_ctx, _directive_tooltip, _ctx.$t("common.addToCart"), void 0, { top: true })))}><i class="pi pi-cart-plus"></i>`);
            _push(ssrRenderComponent(_sfc_main$9, {
              class: "-top-1.5! -right-1.5! !min-w-3 !h-3 !p-0.5",
              item
            }, null, _parent));
            _push(`</button><button${ssrRenderAttrs(mergeProps({ class: "w-5 sm:w-7 h-5 sm:h-7 rounded-lg cursor-pointer flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors" }, ssrGetDirectiveProps(_ctx, _directive_tooltip, _ctx.$t("search.quickView"), void 0, { top: true })))}><i class="pi pi-eye"></i></button></div></div></div></li>`);
          });
          _push(`<!--]--></ul>`);
        }
        if (visibleCount.value < results.value.length) {
          _push(`<button class="cursor-pointer w-full py-3 text-xs font-medium text-brand-500 hover:bg-brand-50 transition-colors">${ssrInterpolate(_ctx.$t("search.showMore"))}</button>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (!isAuthenticated.value) {
        ...
        _push(`<div${ssrRenderAttrs(mergeProps({
          ...
          ref_key: "dropdownRef",
          ref: dropdownRef
        }, _attrs))}><button class="flex items-center gap-2 px-3 sm:py-1.5 rounded-full hover:bg-gray-100 transition-all cursor-pointer select-none"><i class="fa-solid fa-user-gear text-xl! text-brand-500"></i><span class="hidden sm:block text-sm font-medium text-gray-700 max-w-20 truncate">${ssrInterpolate(displayName.value)}</span><i class="${ssrRenderClass([{ "rotate-180": open.value }, "pi pi-chevron-down text-xs text-gray-400 transition-transform duration-200"])}"></i></button>`);
        if (open.value) {
          ...
          ssrRenderList(menuItems.value, (item) => {
            _push(ssrRenderComponent(unref(Link), {
              key: item.label,
              href: item.href ?? _ctx.route(item.route),
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({
        ...
        ref_key: "menuRef",
        ref: menuRef
      }, _attrs))}><button class="flex items-center justify-center w-10 h-10 rounded-full text-gray-600 hover:bg-gray-100 transition-all cursor-pointer"${ssrRenderAttr("aria-label", unref(t)("common.more"))}><i class="pi pi-ellipsis-v text-lg"></i></button>`);
      if (open.value) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (searchOpen.value) {
        ...
        _push(ssrRenderComponent(_sfc_main$5, {
          ...
          placeholder: _ctx.$t("common.searchShort")
        }, null, _parent));
      ...
      _push(`</div><div class="hidden lg:flex items-center gap-1 shrink-0">`);
...
//# sourceMappingURL=index-Dnp6b_Ix.js.map
