...
import { _ as _sfc_main$2, s as script$1 } from "./Breadcrumbs-DHQ88nK8.js";
import { ref, computed, mergeProps, unref, useSSRContext, onMounted, watch, withCtx, createVNode, openBlock, createBlock, createCommentVNode, createSlots, toDisplayString, createTextVNode, Fragment, renderList } from "vue";
import { ssrRenderAttrs, ssrRenderClass, ssrRenderList, ssrRenderAttr, ssrInterpolate, ssrRenderComponent, ssrGetDirectiveProps, ssrIncludeBooleanAttr } from "vue/server-renderer";
import { usePage, Head, Deferred, Link } from "@inertiajs/vue3";
import { useClipboard, onClickOutside } from "@vueuse/core";
import _sfc_main$7 from "./SimilarItems-DE5EoaKD.js";
import _sfc_main$3 from "./ItemGallery-DPazEfKP.js";
import { b as _sfc_main$4, _ as _sfc_main$5 } from "./QuickViewDialog-7zTQs1NP.js";
import { _ as _sfc_main$6 } from "./StockNotifyButton-CruZHgqL.js";
import { _ as _sfc_main$8 } from "./ItemCard-sQ2o_7aA.js";
import { u as useCart } from "./useCart-CIcsIaQl.js";
import { u as usePricing, d as discountedTierPrice } from "./usePricing-ssZPWxiP.js";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({
        ...
        ref_key: "menuRef",
        ref: menuRef
      }, _attrs))}><button type="button" aria-label="გაზიარება" class="${ssrRenderClass([
      ...
      ])}"><i class="pi pi-share-alt text-sm"></i></button>`);
      if (open.value) {
        _push(`<div class="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-lg ring-1 ring-black/5 z-50 overflow-hidden origin-top-right py-1"><!--[-->`);
        ssrRenderList(links.value, (link) => {
          _push(`<a${ssrRenderAttr("href", link.href)} target="_blank" rel="noopener noreferrer" class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors"><i class="${ssrRenderClass([link.icon, link.color, "text-sm w-4 text-center"])}"></i> ${ssrInterpolate(link.label)}</a>`);
        });
        _push(`<!--]--><div class="border-t border-gray-100 mt-1 pt-1"><button type="button" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors cursor-pointer"><i class="${ssrRenderClass(["pi text-sm w-4 text-center", unref(copied) ? "pi-check text-emerald-500" : "pi-copy text-gray-400"])}"></i> ${ssrInterpolate(unref(copied) ? "დაკოპირდა!" : "ბმულის კოპირება")}</button></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.item.prices, (price) => {
        ...
        if (price.priceGroup === "VIP" && _ctx.$page.props.user?.can_view_vip || price.priceGroup !== "VIP" && _ctx.$page.props.user?.can_view_wholesales) {
          ...
          _push(`</div><div class="text-right ml-4"><p class="text-xs text-gray-400 mb-0.5">${ssrInterpolate(__props.item.unit_price === "0.00" ? "შეკვრის ფასი" : "ცალის ფასი")}</p><div class="flex items-center gap-2"><p class="text-brand-500 font-bold text-xl">${ssrInterpolate(Number(unref(discountedTierPrice)(__props.item, price)).toFixed(2))}<span class="text-sm ml-0.5">₾</span></p>`);
          if (price.UOM) {
      ...
      _push(ssrRenderComponent(_component_Panel, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`</span></li><li class="flex gap-2"${_scopeId}><span class="text-brand-500"${_scopeId}>→</span> <span${_scopeId}>მიტანის სერვისის ფარგლებში პროდუქციის მანქანიდან ჩამოტვირთვა და სართულზე ატანა არ შედის მომსახურებაში.</span></li></ul>`);
          } else {
            return [
...
//# sourceMappingURL=Show-B0fjwZuB.js.map
