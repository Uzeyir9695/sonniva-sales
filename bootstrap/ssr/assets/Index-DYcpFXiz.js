  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(items.value, (cartItem) => {
        ...
        _push(`</div><div class="flex-1 min-w-0"><p${ssrRenderAttrs(mergeProps({ class: "text-xs text-gray-700 font-medium line-clamp-1" }, ssrGetDirectiveProps(_ctx, _directive_tooltip, cartItem.item.name, void 0, { top: true })))}>${ssrInterpolate(cartItem.item.name)}</p><p class="text-xs text-gray-400 mt-0.5">${ssrInterpolate(cartItem.qty)} × `);
        if (unref(hasDiscount)(cartItem.item) && cartItem.unitPrice == cartItem.item.discounted_price) {
          _push(`<span class="line-through mr-1">${ssrInterpolate(formatted(cartItem.item.unit_price))} ₾</span>`);
        } else {
          _push(`<!---->`);
        }
        _push(` ${ssrInterpolate(formatted(cartItem.unitPrice))} ₾ </p></div><span class="text-sm font-semibold text-gray-800 shrink-0">${ssrInterpolate(formatted(cartItem.rowTotal))} ₾</span><button${ssrRenderAttrs(mergeProps({
          type: "button",
      ...
      if (totalSavings.value > 0) {
        _push(`<span class="line-through text-gray-400 mr-1">${ssrInterpolate(formatted(subtotal.value + totalSavings.value))} ₾</span>`);
      } else {
        _push(`<!---->`);
      }
...
//# sourceMappingURL=Index-DYcpFXiz.js.map
