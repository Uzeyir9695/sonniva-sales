...
const _sfc_main$1 = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      let _temp0;
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "w-full" }, _attrs))} data-v-f1e5eb62><div data-v-f1e5eb62><label for="city" class="text-sm font-semibold" data-v-f1e5eb62> ზუსტი მისამართი <i${ssrRenderAttrs(_temp0 = mergeProps({ class: "pi pi-exclamation-circle text-sm ml-1 text-red-500" }, ssrGetDirectiveProps(_ctx, _directive_tooltip, "სავალდებულო ველი", void 0, { top: true })))} data-v-f1e5eb62>${"textContent" in _temp0 ? ssrInterpolate(_temp0.textContent) : _temp0.innerHTML ?? ""}</i></label>`);
      if (showWarning.value) {
        _push(`<p class="text-sm text-red-500 mt-1 mb-1" data-v-f1e5eb62><i class="pi pi-exclamation-circle text-xs" data-v-f1e5eb62></i> აირჩიეთ შესაბამისი მისამართი ქვემოთ მოცემული ლისტიდან </p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="relative mt-2" data-v-f1e5eb62>`);
      ...
      if (isConfirmed.value) {
        _push(`<span class="absolute right-2.5 top-1/2 -translate-y-1/2 text-green-500 pointer-events-none" data-v-f1e5eb62><i class="pi pi-check-circle text-base" data-v-f1e5eb62></i></span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div>`);
      if (suggestions2.value.length) {
        _push(`<ul class="border rounded-md bg-white mt-2 shadow-md" data-v-f1e5eb62><!--[-->`);
        ssrRenderList(suggestions2.value, (item, i) => {
          _push(`<li class="p-2 hover:bg-gray-100 cursor-pointer" data-v-f1e5eb62><strong data-v-f1e5eb62>${ssrInterpolate(item.placePrediction.structuredFormat?.mainText?.text || item.placePrediction.text?.text)}</strong>`);
          if (item.placePrediction.structuredFormat?.secondaryText) {
            _push(`<br data-v-f1e5eb62>`);
          } else {
            _push(`<!---->`);
          }
          if (item.placePrediction.structuredFormat?.secondaryText) {
            _push(`<small data-v-f1e5eb62>${ssrInterpolate(item.placePrediction.structuredFormat.secondaryText.text)}</small>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</li>`);
...
const PlacesAutocomplete = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-f1e5eb62"]]);
...
//# sourceMappingURL=Index-BfdiI8S_.js.map
