...
import { _ as _sfc_main$1 } from "./StockNotifyDialog-k_i0eSJl.js";
var inlineStyles = {
  root: {
    position: "relative"
  }
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!__props.isSubscribed) {
        _push(`<button class="w-full py-1.5 sm:py-2 rounded-2xl border-2 border-dashed border-slate-400 text-slate-700 text-sm font-bold hover:bg-slate-50 transition-colors cursor-pointer flex items-center justify-center gap-2"><i class="pi pi-bell font-bold"></i> ${ssrInterpolate(_ctx.$t("stockNotify.notifyWhenBack"))}</button>`);
      } else {
        _push(`<div class="rounded-2xl bg-brand-100 border border-brand-300 px-4 py-3"><div class="flex items-center justify-between gap-3"><div class="flex items-center gap-2 text-brand-800 text-sm font-medium"><i class="pi pi-bell text-brand-600"></i> ${ssrInterpolate(_ctx.$t("stockNotify.enabled"))}</div><button class="text-xs text-brand-600 hover:text-red-500 transition-colors cursor-pointer shrink-0">${ssrInterpolate(_ctx.$t("common.cancel"))}</button></div><p class="text-xs text-brand-700 mt-1.5">${ssrInterpolate(_ctx.$t("stockNotify.backSmsNote"))}</p></div>`);
      }
...
//# sourceMappingURL=StockNotifyButton-EPPGxV3D.js.map
