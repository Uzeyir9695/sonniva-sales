...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--></div><section class="bg-white rounded-2xl border border-emerald-200 shadow-sm overflow-hidden"><div class="bg-emerald-50 px-6 py-4 flex items-center gap-3 border-b border-emerald-100"><i class="pi pi-building text-emerald-600 text-lg"></i><div><h2 class="font-bold text-gray-900 text-base">სონნივას ფილიალიდან გატანა</h2><p class="text-xs text-emerald-600 font-medium mt-0.5">სრულიად უფასო</p></div><span class="ml-auto text-xl font-bold text-emerald-600">0 ₾</span></div><div class="p-6 grid sm:grid-cols-2 gap-4"><!--[-->`);
      ssrRenderList(officeBranches, (branch) => {
        _push(`<div class="flex items-start gap-2 text-sm text-gray-600 rounded-xl border border-gray-200 ring-1 ring-gray-100 p-4"><i class="pi pi-map-marker text-brand-500 mt-0.5 shrink-0"></i><div class="space-y-1"><p class="font-medium text-gray-800">${ssrInterpolate(branch.address)}</p><p class="text-xs text-gray-400 mt-0.5">${ssrInterpolate(branch.hours)}</p><a${ssrRenderAttr("href", branch.mapUrl)} target="_blank" class="text-brand-500 underline"> მისამართის სანახავად დააჭირეთ ლინკს </a></div></div>`);
      });
      _push(`<!--]--></div></section><section class="space-y-4"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-xl bg-blue-50 flex items-center justify-center"><i class="pi pi-map-marker text-blue-600"></i></div><div><h2 class="font-bold text-gray-900 text-base">მიწოდება თბილისში</h2><p class="text-xs text-gray-400">ფასი განისაზღვრება ზონის მიხედვით</p></div></div><div class="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 flex items-center gap-2 text-sm text-emerald-700"><i class="pi pi-check-circle shrink-0"></i><span><strong>${ssrInterpolate(TBILISI_FREE_THRESHOLD)} ₾-ზე</strong> მეტი შეკვეთისას — თბილისში მიწოდება <strong>უფასოა</strong></span></div><div class="grid grid-cols-1 sm:grid-cols-3 gap-4"><!--[-->`);
...
//# sourceMappingURL=DeliveryRates-MAvASpCE.js.map
