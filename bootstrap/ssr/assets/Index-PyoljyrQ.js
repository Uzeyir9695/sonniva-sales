import { computed, mergeProps, unref, withCtx, createVNode, openBlock, createBlock, Fragment, renderList, useSSRContext, ref, toDisplayString, createCommentVNode, createTextVNode, resolveDynamicComponent } from "vue";
      ...
      if (__props.section.items?.length || __props.section.images?.length) {
        ...
        if (__props.section.images?.length) {
          ...
          ssrRenderList(__props.section.images, (image) => {
            ssrRenderVNode(_push, createVNode(resolveDynamicComponent(image.link_url ? "a" : "div"), {
              key: image.id,
              ...
              default: withCtx((_, _push2, _parent2, _scopeId) => {
                if (_push2) {
                  _push2(`<img${ssrRenderAttr("src", image.image_url)}${ssrRenderAttr("alt", image.title || "")} class="w-full h-full object-cover"${_scopeId}>`);
                  if (image.title) {
                    _push2(`<span class="absolute bottom-0 inset-x-0 px-3 py-2 text-sm font-medium text-white bg-black/50 truncate"${_scopeId}>${ssrInterpolate(image.title)}</span>`);
                  } else {
                    _push2(`<!---->`);
                  }
                } else {
...
//# sourceMappingURL=Index-PyoljyrQ.js.map
